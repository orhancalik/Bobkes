...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    const morePokemonBtn = document.getElementById("morePokemonBtn") as HTMLElement;
    ...
    async function fetchAndDisplayPokemons(offset: number): Promise<void> {
        try {
            ...
            const currentPokemonCount = pokedexList?.querySelectorAll(".MijnPokemonLijst").length || 0;
    ...
    morePokemonBtn.addEventListener("click", async function () {
        offset += 50;
        await fetchAndDisplayPokemons(offset);
    });
    ...
    // Initial fetch to load the first set of Pokémon
    await fetchAndDisplayPokemons(offset);
});
